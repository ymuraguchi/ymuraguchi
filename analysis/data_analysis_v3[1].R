...
table <- data %>%
  kbl(
    caption = "スポーツ競技における主観的パフォーマンス測定指標の下位尺度とPPFI-Jの下位尺度の相関係数",
    booktabs = TRUE,
    align = "c"
  ) %>%
  kable_styling(full_width = FALSE, position = "center") %>%
  add_header_above(c(" " = 3, "相関係数" = 6)) %>%
  footnote(
    general = "***p < .001, **p < .01, *p < .05",
    general_title = "注:",
    footnote_as_chunk = TRUE
  )

# 表をHTMLファイルに保存
save_kable(as.character(table), file = "table_output.html")

# 必要なパッケージ
install.packages("webshot")
webshot::install_phantomjs()  # 初回のみ必要

# HTMLをPDFに変換
webshot::webshot("table_output.html", file = "table_output.pdf")
